...
const handleSubmitTransaksi = async (e) => {
    ...
    try {
      // 1. Ambil data user dari localStorage (atau berikan fallback ID = 1)
      ...
      if (userStorage) {
        try {
          ...
          // Ekstrak ID (bertipe angka) agar tidak mengirim objek [object Object]
          userId = parsedUser.id || parsedUser.user_id || 1;
        } catch (err) {
          console.error("Gagal parse user dari localStorage:", err);
        }
      }

      // 2. Sertakan user_id bertipe integer ke dalam payload
      const payload = {
        ...form,
        user_id: parseInt(userId, 10), // Pastikan bertipe integer murni
        items: detail
      };
      ...
      if (res.data?.success) {
        ...
        // FIX TYPO: Perbaikan pelangganTerpielen -> pelangganTerpilih
        const namaCustomerDisplay = pelangganTerpilih 
          ? `${pelangganTerpilih.nama_customer} ${pelangganTerpilih.perusahaan_mitra ? `(${pelangganTerpilih.perusahaan_mitra})` : ""}`
          : "Pelanggan Umum (Cash/Tunai)";

        // SINKRONISASI TOTAL
        const dataInvoiceFinal = {
          penjualan: {
            id: res.data.penjualan_id, 
