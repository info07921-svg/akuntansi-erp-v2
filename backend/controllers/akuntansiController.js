...
exports.getBukuBesar = async (req, res) => {
  try {
    ...
    // 1. Ambil info akun (Dukungan ID maupun Kode Akun '111001')
    const [akunInfo] = await db.query(
      `SELECT * FROM akun WHERE (id = ? OR kode_akun = ?) AND (perusahaan_id = ? OR perusahaan_id IS NULL)`,
      [akun_id, akun_id, perusahaan_id]
    );

    if (akunInfo.length === 0) {
      return res.json({ success: true, data: [] });
    }

    const realAkunId = akunInfo[0].id;
    const tipeAkun = String(akunInfo[0].tipe || "").toUpperCase();

    // 2. Query Detail Jurnal (Melonggarkan filter perusahaan_id agar data legacy/null tetap terbaca)
    let sql = `
      SELECT
      ...
      WHERE detail_jurnal.akun_id = ?
    `;

    const params = [realAkunId];
    ...
    // 3. Kalkulasi Akumulasi Saldo Berjalan
    let saldoBerjalan = 0;
    const formattedData = rows.map((row) => {
      ...
      if (["KAS", "BANK", "ASET", "BEBAN"].includes(tipeAkun)) {
        saldoBerjalan += (debitVal - kreditVal);
      } else {
        saldoBerjalan += (kreditVal - debitVal);
      }
